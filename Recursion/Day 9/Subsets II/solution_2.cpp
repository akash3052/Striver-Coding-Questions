...
public:
    ...
    void solve(set<vector<int>> &res, vector<int> input, vector<int> output){
        if(input.size() == 0){
            // Since, we have not sort the input, sorting here so that all possible combination of subsets will become same(e.g. [4,1,4], [1,4,4] and [4,4,1] -> [1,4,4])
    ...
    vector<vector<int>> subsetsWithDup(vector<int>& nums) {
        set<vector<int>> res;
        vector<int> output; 
        // Note -> Not sorting the input, will sort the output before inserting the output in set.
